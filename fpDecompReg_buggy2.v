...
// Decompose input
always @(posedge clk)
	if (!ce) begin // BUG 1: Inverted chip enable
		o <= i;
		sgn = i[MSB];
		exp = i[MSB-1:FMSB+1];
		man = i[FMSB:0];
		...
		xinf = &exp;
		qnan = &exp &  man[FMSB];
        // missing other signals due to truncation...
    end
    
    // BUG 2: Using blocking assignments in sequential block (for registers)
    // already there in original? No, it's mixed. Let's make it worse:
    // Actually the original uses = for most signals except o. Let's make fract update incorrectly.
    always @(posedge clk)
        if (ce) fract = {xz, i[FMSB:0]}; // BUG 2: Incorrect hidden bit restoration
